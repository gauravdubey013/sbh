...
const MakePayment = (props) => {
    ...
    //     printRef.current.offsetWidth / 210, // width of printRef div in mm
    //     printRef.current.offsetHeight / 297 // height of printRef div in mm
    // );
    ...
    const makePayment = async (e) => {
        e.preventDefault();
        setDisableSubmit(true);
        if (amount !== "" && upiId !== "" && pinCode !== "") {
            // console.log(amount, upiId, pinCode);
            const formData = JSON.stringify({ userEmail, profEmail, amount, upiId });
            try {
        ...
        <>
            <section className="w-full h-full relative flex items-center justify-center overflow-hidden">
                <ReactToPrint
                    ...
                    trigger={() =>
                        <div ref={printRef} className={`${printReceiptVisible ? "animate-slideDown" : "hidden"} absolute cursor-pointer w-[95%] h-[95%] bg-[url('/assets/receiptBg2.png')] bbg bg-cover z-10 px-2 py-1 rounded-lg`}>
                            <div className="w-full flex justify-between text-[#4cffa5]">
                                <h2 className='text-4xl font-extrabold text-start'>Payment Receipt</h2>
                                <h2 className='text-xl font-extrabold text-end'>SkillBeHired</h2>
                            </div>
                            <span className='absolute right-1 top-[4.8rem] text-[12px]'>Date : {fetchedDetails?.date ?? "NaN"}</span>
                            <div className="w-full h-[63.5%] borde mt-[10.5vh] text-start flex flex-col text-[16px]">
                                ...
                                <span className='absolute bottom-12 borde text-[11px] leading-tight text-justify'>After your work done acceptance decision, we will pay remaining amount to professional (if " <u>YES</u> ") else pay-back to you through your UPI ID. You can make decision in professional's chat section, it will appear when professional request for it.</span>
                            </div>
                    ...
                    <form action="" onSubmit={makePayment} className='w-full h-full flex flex-col gap-1 justify-between'>
                        ...
                        <div className="w-full flex flex-col gap-1">
                            <div className="flex items-center">
                                <span className='text-[#4cffa5]'>₹</span><input type="text" value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^\d]/g, ""))} required placeholder='Enter Amount' className='allFormInput h-[52px]' />
                            </div>
                            {amount !== "" &&
